...
export const Card = ({ profile_path, name, character }) => {
  return (
    <div className="flex flex-col  justify-between w-full h-full">
      ...
      <div className="flex flex-col  gap-5  h-1/2  overflow-ellipsis line-clamp-2" >
        <p className=" text-center font-bold text-white text-md">{name ?? "Sin nombre"}</p>
        <p className=" text-center font-bold text-white text-sm">
          Personaje: {character ?? "Sin nombre"}
        </p>
      </div>
    </div>
  );
